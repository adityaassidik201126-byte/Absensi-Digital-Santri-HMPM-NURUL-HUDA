...
<body>
    ...
    <div id="sidebar">
        <div class="sidebar-header">
            ...
            <p class="sidebar-role">👨‍🎓 Role: Santri/Siswa (Akses Penuh)</p>
        </div>
    ...
    <div id="main-content">
        ...
        <div id="dashboard-view" class="view active">
            ...
            <div id="schedule-control-dashboard" style="max-width: 400px; margin: 0 auto; padding: 20px; background-color: white; border-radius: 8px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.05);">
                ...
                <button id="btn-activate" class="btn btn-active" onclick="setScheduleMode(true)" style="background-color: #28a745; color: white; border: none; padding: 10px 15px; border-radius: 5px; cursor: pointer;">✅ Aktifkan Jadwal</button>
                <button id="btn-deactivate" class="btn btn-inactive" onclick="setScheduleMode(false)" style="background-color: #dc3545; color: white; border: none; padding: 10px 15px; border-radius: 5px; cursor: pointer;">❌ Nonaktifkan Jadwal</button>
                <p>Mode Saat Ini: <strong id="mode-display">AKTIF (Sesuai Jadwal)</strong></p>
            </div>
        </div>

        <div id="scanner-view" class="view">
             <h1 style="color: #1a5c34;">Absensi Santri (QR) 📸</h1>
        ...
        <div id="manual-view" class="view">
            <h1 style="color: #6c757d;">Absensi Manual ✍️</h1>
    ...
    <script>
        ...
        function renderBelumHadirList() {
            ...
            if (isAllPresent) {
                listContent.innerHTML = '<p class="belum-hadir-empty" style="color: #28a745; font-size: 1.2em; font-style: normal;"><i class="fas fa-thumbs-up"></i> Semua santri sudah tercatat hadir! 🎉</p>';
        ...
        function sendBelumHadirReport() {
            ...
            let reportText = `*⚠️ PEMBERITAHUAN ABSENSI BELUM HADIR ⚠️*\n`;
        ...
        function renderClassRecap() {
            ...
            let recapHTML = `
                <h2 style="color: #333; border-bottom: 2px solid #ccc; padding-bottom: 5px; margin-bottom: 20px; font-size: 1.5em;">📊 Ringkasan Kehadiran Santri Per Kelas</h2>
                ...
                recapHTML += `
                    <div class="stat-card" style="padding: 15px; background-color: #ffffff; border-radius: 8px; box-shadow: 0 4px 8px rgba(0,0,0,0.05); border-left: 5px solid ${belumHadirCount > 0 ? '#dc3545' : '#28a745'};">
                        ...
                        <p style="font-size: 0.9em; margin: 0; padding: 5px; background-color: #f4f7f9; border-radius: 4px;">
                            ${belumHadirNames.length > 0 ? belumHadirNames.join(', ') : 'Semua sudah hadir! 🎉'}
                        </p>
                    </div>
                `;
            }
        ...
        function recordManualAttendance() {
            ...
            if (!selectedId) {
                updateManualResultDisplay(`❌ GAGAL: Mohon pilih nama santri.`, '#f8d7da', '#721c24');
                return;
            }
            ...
            if (recordedURLs.has(studentUrl)) {
                updateManualResultDisplay(`⚠️ PERINGATAN: ${studentData.name} (${studentData.class}) sudah tercatat absensi hari ini.`, '#fff3cd', 'orange');
                return;
            }
            ...
            updateManualResultDisplay(
                `✅ SUKSES: Absensi ${studentData.name} (${finalStatusText}) dicatat secara MANUAL.`, 
                '#d4edda', 
                '#155724'
            );
        ...
        function onScanSuccess(decodedText, decodedResult) {
            ...
            if (!santriID) {
                updateResultDisplay(`❌ GAGAL: URL tidak terdaftar!`, '#f8d7da', '#721c24');
            ...
            resultDisplayTimeout = setTimeout(() => {
                ...
                if (isAlreadyRecorded) {
                    const alertMsg = `⚠️ ${studentName} (${studentClass}) Sudah tercatat.`; 
                    updateResultDisplay(alertMsg, '#fff3cd', '#856404'); 
                    resultDisplayTimeout = setTimeout(clearResultDisplay, 1000);

                } else if (isScheduleActive && attendanceStatus === "Di Luar Jadwal") {
                    const alertMsg = `❌ GAGAL: Absensi ${studentName} dilakukan ${attendanceStatus}.`; 
                    ...
                    updateResultDisplay(
                        `✅ ${studentName} (${studentClass}) - ${finalStatus}`, 
                        successBg, 
                        successColor
                    );
        ...
        function saveJadwal() {
            ...
            resultDiv.innerText = `✅ Jadwal Absen berhasil diperbarui! Mulai: ${startTime}, Batas Terlambat: ${lateTime}, Akhir: ${endTime}`;
        ...
        function addNewUser() {
            ...
            resultDiv.style.color = '#28a745';
            resultDiv.innerText = `✅ Akun baru "${name}" dengan Role: ${role} berhasil ditambahkan (Simulasi)!`;
        ...
        function addSantri(id, name, selectedClass) {
            ...
            renderSantriManagementTable();
            alert(`✅ Santri ${name} (${id}) berhasil ditambahkan ke kelas ${selectedClass}!`);
        }
        ...
        function updateSantri(originalId, newId, newName, newClass) {
            ...
            renderSantriManagementTable();
            alert(`✅ Data Santri ${newName} (${newId}) berhasil diperbarui!`);
        }
        ...
        function deleteSantri(id, cls) {
            ...
            if (index !== -1) {
                ...
                alert(`✅ Santri ${id} berhasil dihapus.`);
            }
        }
